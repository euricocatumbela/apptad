...
const AmplifySignUp = class {
    ...
    async signUp(event) {
        ...
        if (this.signUpAttributes.username.indexOf(' ') >= 0) {
            helpers.dispatchToastHubEvent(new Error(Translations.Translations.USERNAME_REMOVE_WHITESPACE));
        }
        if (this.signUpAttributes.password !== this.signUpAttributes.password.trim()) {
            helpers.dispatchToastHubEvent(new Error(Translations.Translations.PASSWORD_REMOVE_WHITESPACE));
        }
        try {
